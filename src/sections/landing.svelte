...
>
	...
	>
		...
		>
			<p bind:this={helloEl} class="text-md block text-center sm:text-xl md:text-left">
				{helloText}
			</p>
			<h2 class="block text-center text-4xl font-semibold sm:text-6xl md:text-left">
				<span bind:this={introEl}>{introText}</span>
				<span bind:this={nameEl} class="text-[#dbefec] text-shadow-lg/60 text-shadow-[#dbefec]/60"
					>{nameText}</span
				>
			</h2>
			<p bind:this={roleEl} class="text-md max-w-fit text-center sm:text-xl md:text-left">
				{roleText}
			</p>
		</div>
	</div>
